...
mod tests {
    ...
    fn swap_d0() {
        ...
        mm.cpu.register.set_d_reg_long_no_log(0, 0xBBFF66AA);
        ...
        mm.step();
        // assert
        assert_eq!(0x66AABBFF, mm.cpu.register.get_d_reg_long_no_log(0));
    ...
    fn swap_d0_negative() {
        ...
        let code = [0x48, 0x40].to_vec(); // SWAP D0
        let mut mm = crate::tests::instr_test_setup(code, None);
        mm.cpu.register.set_d_reg_long_no_log(0, 0x12908000);
        mm.cpu.register.reg_sr.set_sr_reg_flags_abcde(0x0000);
        // act assert - debug
        let debug_result = mm.get_next_disassembly_no_log();
        assert_eq!(
        ...
        mm.step();
        // assert
        assert_eq!(0x80001290, mm.cpu.register.get_d_reg_long_no_log(0));
        assert_eq!(0xC00002, mm.cpu.register.reg_pc.get_address());
        assert_eq!(true, mm.cpu.register.reg_sr.is_sr_carry_set());
        assert_eq!(true, mm.cpu.register.reg_sr.is_sr_overflow_set());
        assert_eq!(false, mm.cpu.register.reg_sr.is_sr_zero_set());
